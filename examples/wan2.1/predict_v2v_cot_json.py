...
def load_video_frames(
    ...
    assert source_frames is not None, "source_frames must be provided"
...
def parse_args():
    ...
    parser.add_argument("--model_name", type=str, default="Wan2.1-T2V-14B", help="Model checkpoint path")
...
def main():
    ...
    if isinstance(eval_prompts_list, dict):
        eval_prompts_list = list(eval_prompts_list.values())
    ...
    transformer = WanTransformer3DModel.from_pretrained(
        resolve_model_path(model_name, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer'), 'transformer'),
    ...
    vae = AutoencoderKLWan.from_pretrained(
        resolve_model_path(model_name, config['vae_kwargs'].get('vae_subpath', 'vae'), 'vae'),
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        resolve_model_path(model_name, config['text_encoder_kwargs'].get('tokenizer_subpath', 'tokenizer'), 'tokenizer'),
    )

    text_encoder = WanT5EncoderModel.from_pretrained(
        resolve_model_path(model_name, config['text_encoder_kwargs'].get('text_encoder_subpath', 'text_encoder'), 'text_encoder'),
    ...
    for fname, item in subset_items:
        ...
        edit_text = item.get('text', item.get('qwen_vl_72b_refined_instruction', item.get('edit_instruction', item.get('instruction', ''))))
