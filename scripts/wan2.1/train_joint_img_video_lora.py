...
from videox_fun.data.dataset_image_video import (ImageVideoDataset,
                                                ImageVideoEditDataset,
                                                ImageVideoCoTDataset,
                                                ImageVideoSampler,
                                                VideoEditDataset,
    ...
    class WanI2VPipeline:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise ImportError(
                ...
                "Use the default train_mode='normal' command for VideoCoF CoT LoRA training."
            )
from videox_fun.utils.discrete_sampler import DiscreteSampling
from videox_fun.utils.lora_utils import create_network, merge_lora, unmerge_lora
from videox_fun.utils.utils import get_image_to_video_latent, save_videos_grid

from validation import run_final_ddp_validation
...
def parse_args():
    ...
    parser.add_argument(
        "--run_final_ddp_validation",
        action="store_true",
        help="After training finishes, run a final distributed validation/test on the same hardware.",
    )
    parser.add_argument(
        "--validation_json",
        type=str,
        default=None,
        help="Path to validation task json for post-training DDP validation.",
    )
    parser.add_argument(
        "--validation_output_dir",
        type=str,
        default=None,
        help="Output directory for validation results (defaults to <output_dir>/final_validation).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_image_video_edit_dataset", action="store_true", help="Whether enable image video edit dataset.",
    )
    parser.add_argument(
        "--source_frames",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use ImageVideoCoTDataset (for videos: triplet with original/grounded/edited; for images: pair with original/edited).",
    )
    parser.add_argument(
        "--use_nhr_dataset",
        action="store_true",
        default=False,
        help="Use NHREditDataset (Parquet format support).",
    )
    parser.add_argument(
        ...
        default=False,
        help=(
            "Enable gradually-grounded frames: interpolate between frame 0 of grounded and edited videos "
            "with 16 steps, picking indices 0,4,8,12 as the 4 grounding frames."
        ),
    )
    parser.add_argument(
        "--enable_gray_red_mask",
        action="store_true",
        default=False,
        help="Enable gray-region detection on CoT reasoning frames and overlay detected regions with solid red.",
    )
    parser.add_argument(
        "--enable_gray_black_background",
        action="store_true",
        default=False,
        help="Enable gray-region detection on CoT reasoning frames and keep detected regions while turning background black.",
    )
    parser.add_argument(
        "--enable_gray_alpha_overlay",
        action="store_true",
        default=False,
        help="Make CoT reasoning frames grayscale and blend with original using --gray_alpha.",
    )
    parser.add_argument(
        "--enable_gray_bounding_box",
        action="store_true",
        default=False,
        help="Detect gray regions in CoT reasoning frames and apply gradient gray overlay to their bounding box (weaker supervision).",
    )
    parser.add_argument(
        "--gray_alpha",
        type=float,
        default=0.5,
        help="Alpha for grayscale overlay on CoT reasoning frames (0..1).",
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    # Ensure only one CoT reasoning visualization mode is enabled
    enabled_modes = int(bool(args.enable_gray_red_mask)) + int(bool(args.enable_gray_black_background)) + int(bool(args.enable_gray_alpha_overlay)) + int(bool(args.enable_gray_bounding_box))
    if enabled_modes > 1:
        parser.error("--enable_gray_red_mask, --enable_gray_black_background, --enable_gray_alpha_overlay and --enable_gray_bounding_box cannot be set simultaneously.")
    if not (0.0 <= float(args.gray_alpha) <= 1.0):
        parser.error("--gray_alpha must be within [0.0, 1.0].")
...
def main():
    ...
    logger.info(f"[sanity] accelerator.mixed_precision={accelerator.mixed_precision}")
    ...
    if accelerator.mixed_precision == "fp16":
        ...
        args.mixed_precision = accelerator.mixed_precision
    logger.info(f"[sanity] training weight_dtype={weight_dtype}")
    ...
    logger.info(
        "[sanity] use_cot_dataset=%s, enable_gradual_ground=%s, enable_gray_red_mask=%s, enable_gray_black_background=%s, enable_gray_alpha_overlay=%s, enable_gray_bounding_box=%s, gray_alpha=%.2f",
        args.use_cot_dataset,
        args.enable_gradual_ground,
        args.enable_gray_red_mask,
        args.enable_gray_black_background,
        args.enable_gray_alpha_overlay,
        args.enable_gray_bounding_box,
        args.gray_alpha,
    )

    if args.use_cot_dataset:
        train_dataset = ImageVideoCoTDataset(
            ann_path=args.train_data_meta,
            data_root=args.train_data_dir,
            video_sample_size=args.video_sample_size,
            video_sample_stride=args.video_sample_stride,
            source_frames=args.source_frames,      # e.g., 33
            reasoning_frames=args.reasoning_frames, # e.g., 4
            target_frames=args.edit_frames,         # e.g., 32
            text_drop_ratio=0.1,
            enable_bucket=args.enable_bucket,
            enable_inpaint=True if args.train_mode != "normal" else False,
            enable_gradual_ground=args.enable_gradual_ground,
            enable_gray_red_mask=args.enable_gray_red_mask,
            enable_gray_black_background=args.enable_gray_black_background,
            enable_gray_alpha_overlay=args.enable_gray_alpha_overlay,
            enable_gray_bounding_box=args.enable_gray_bounding_box,
            gray_alpha=args.gray_alpha,
        )
    elif args.use_nhr_dataset:
        train_dataset = NHREditDataset(
            ann_path=args.train_data_meta,
            data_root=args.train_data_dir,
            video_sample_size=args.video_sample_size,
            source_frames=args.source_frames,
            target_frames=args.edit_frames,
            text_drop_ratio=0.1,
            enable_bucket=args.enable_bucket,
            enable_inpaint=True if args.train_mode != "normal" else False,
        )
    elif args.use_image_video_edit_dataset:
        train_dataset = ImageVideoEditDataset(
            ann_path=args.train_data_meta,
            data_root=args.train_data_dir,
            video_sample_size=args.video_sample_size,
            video_sample_stride=args.video_sample_stride,
            source_frames=args.source_frames,  # e.g., 17
            target_frames=args.edit_frames,     # e.g., 16
            text_drop_ratio=0.1,
            enable_bucket=args.enable_bucket,
            enable_inpaint=True if args.train_mode != "normal" else False,
        )
    elif args.use_reasoning_dataset:
    ...
    if args.enable_bucket:
        ...
        def collate_fn_paired(examples):
            # Get token length
            ...
            # Create new output for paired data
            new_examples = {}
            new_examples["target_token_length"] = target_token_length
            new_examples["pixel_values_src"] = []
            new_examples["pixel_values_tgt"] = []
            new_examples["text"] = []
            new_examples["frame_split_indices"] = []  # Track src frame count for each sample
            
            # Check if this is CoT dataset (has ground video)
            first_example = examples[0]
            is_cot_data = "pixel_values_ground_video" in first_example
            if is_cot_data:
                new_examples["pixel_values_ground"] = []
                new_examples["ground_frame_indices"] = []  # Track ground frame positions
            
            # Verify all examples in batch are same type (enforced by sampler)
            data_type = first_example["data_type"]
            data_types = [ex["data_type"] for ex in examples]
            if len(set(data_types)) > 1:
                raise ValueError(f"Mixed batch detected! All examples must be same type. Got: {data_types}")
            
            # Get pixel values shape for determining aspect ratio
            if data_type == "video":
                src_pixels = first_example["pixel_values_src_video"]
                if is_cot_data:
                    ground_pixels = first_example["pixel_values_ground_video"]
                tgt_pixels = first_example["pixel_values_tgt_video"]
            else:
                src_pixels = first_example["pixel_values_src_image"]
                tgt_pixels = first_example["pixel_values_tgt_image"]
            
            # Assume src and tgt have same shape
            if isinstance(src_pixels, np.ndarray):
                f, h, w, c = np.shape(src_pixels)
            else:
                f, c, h, w = src_pixels.shape
            
            # Determine batch video length and aspect ratio handling
            if data_type == 'image' and f == 1:
                # Single-frame image
                ...
                # Video or multi-frame
                if args.random_hw_adapt:
                    if args.training_with_video_token_length:
                        # Calculate based on all examples
                        local_min_size = np.min(np.array([
                            np.mean(np.array([
                                np.shape(ex["pixel_values_src_video"] if ex["data_type"]=="video" else ex["pixel_values_src_image"])[1],
                                np.shape(ex["pixel_values_src_video"] if ex["data_type"]=="video" else ex["pixel_values_src_image"])[2]
                            ])) for ex in examples
                        ]))
            ...
            # Determine target size for resizing/cropping
            ...
            for example in examples:
                ...
                if data_type == "video":
                    src_pixels = example["pixel_values_src_video"]
                    if is_cot_data:
                        ground_pixels = example["pixel_values_ground_video"]
                    tgt_pixels = example["pixel_values_tgt_video"]
                else:
                    src_pixels = example["pixel_values_src_image"]
                    tgt_pixels = example["pixel_values_tgt_image"]
                
                # Convert numpy to tensor if needed
                if isinstance(src_pixels, np.ndarray):
                    src_pixels = torch.from_numpy(src_pixels).permute(0, 3, 1, 2).contiguous() / 255.
                    if is_cot_data and data_type == "video":
                        ground_pixels = torch.from_numpy(ground_pixels).permute(0, 3, 1, 2).contiguous() / 255.
                    tgt_pixels = torch.from_numpy(tgt_pixels).permute(0, 3, 1, 2).contiguous() / 255.
                ...
                src_pixels = transform(src_pixels)
                if is_cot_data and data_type == "video":
                    ground_pixels = transform(ground_pixels)
                tgt_pixels = transform(tgt_pixels)
                
                # Update batch_video_length to minimum of current lengths
                if is_cot_data and data_type == "video":
                    batch_video_length = int(min(batch_video_length, len(src_pixels), len(tgt_pixels)))
                else:
                    batch_video_length = int(min(batch_video_length, len(src_pixels), len(tgt_pixels)))
                
                # Record src frame count for temporal RoPE split
                # For CoT data: split happens after src, predicting ground+tgt
                # For regular paired data: split happens after src, predicting tgt
                new_examples["frame_split_indices"].append(len(src_pixels))
                
                # Record ground frame range if CoT data
                if is_cot_data and data_type == "video":
                    # Ground frames are between src and tgt
                    ground_start = len(src_pixels)
                    ground_end = ground_start + len(ground_pixels)
                    new_examples["ground_frame_indices"].append((ground_start, ground_end))
                
                new_examples["pixel_values_src"].append(src_pixels)
                if is_cot_data and data_type == "video":
                    new_examples["pixel_values_ground"].append(ground_pixels)
                new_examples["pixel_values_tgt"].append(tgt_pixels)
                new_examples["text"].append(example["text"])
            
            # Align frames to 4n+1 for MagVAE
            batch_video_length = (batch_video_length - 1) // sample_n_frames_bucket_interval * sample_n_frames_bucket_interval + 1
            if batch_video_length <= 0:
                batch_video_length = 1
            
            # Truncate or pad all examples to same frame count
            padded_src = []
            padded_ground = [] if is_cot_data else None
            padded_tgt = []
            
            if is_cot_data and "pixel_values_ground" in new_examples and len(new_examples["pixel_values_ground"]) > 0:
                # CoT data with ground
                # Note: ground frames (usually 4 frames) are kept as-is, only src and tgt are padded/truncated
                for src_ex, ground_ex, tgt_ex in zip(new_examples["pixel_values_src"], new_examples["pixel_values_ground"], new_examples["pixel_values_tgt"]):
                    if len(src_ex) < batch_video_length:
                        # Pad by repeating last frame for src and tgt only
                        pad_frames = batch_video_length - len(src_ex)
                        src_ex = torch.cat([src_ex, src_ex[-1:].repeat(pad_frames, 1, 1, 1)], dim=0)
                        tgt_ex = torch.cat([tgt_ex, tgt_ex[-1:].repeat(pad_frames, 1, 1, 1)], dim=0)
                    elif len(src_ex) > batch_video_length:
                        # Truncate src and tgt only
                        src_ex = src_ex[:batch_video_length]
                        tgt_ex = tgt_ex[:batch_video_length]
                    
                    padded_src.append(src_ex)
                    padded_ground.append(ground_ex)  # Keep ground as-is (e.g., 4 frames)
                    padded_tgt.append(tgt_ex)
            else:
                # Regular paired data without ground
                for src_ex, tgt_ex in zip(new_examples["pixel_values_src"], new_examples["pixel_values_tgt"]):
                    if len(src_ex) < batch_video_length:
            ...
            new_examples["pixel_values_src"] = torch.stack(padded_src)
            if padded_ground is not None and len(padded_ground) > 0:
                new_examples["pixel_values_ground"] = torch.stack(padded_ground)
            new_examples["pixel_values_tgt"] = torch.stack(padded_tgt)
        ...
        def collate_fn(examples):
            # Get token length
            is_paired_data = "pixel_values_src_video" in examples[0] or "pixel_values_src_image" in examples[0]
            
            if is_paired_data:
                return collate_fn_paired(examples)
            else:
                target_token_length = args.video_sample_n_frames * args.token_sample_size * args.token_sample_size
                length_to_frame_num = get_length_to_frame_num(target_token_length)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            if epoch == first_epoch and step == 0:
                ...
                is_paired_data = "pixel_values_src" in batch
                
                if is_paired_data:
                    # For paired data, concatenate source and target (and ground if exists) for visualization
                    pixel_values_src = batch['pixel_values_src'].cpu()
                    pixel_values_tgt = batch['pixel_values_tgt'].cpu()
                    
                    # Check if this is CoT data with ground
                    if "pixel_values_ground" in batch:
                        pixel_values_ground = batch['pixel_values_ground'].cpu()
                        # CoT: concatenate src + ground + tgt
                        pixel_values = torch.cat([pixel_values_src, pixel_values_ground, pixel_values_tgt], dim=1)
                    else:
                        # Regular paired: concatenate src + tgt
                        pixel_values = torch.cat([pixel_values_src, pixel_values_tgt], dim=1)
                ...
                if args.train_mode != "normal" and not is_paired_data:
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if is_paired_data:
                    # Get source and target pixels, bf16
                    pixel_values_src = batch["pixel_values_src"].to(accelerator.device, dtype=weight_dtype)
                    pixel_values_tgt = batch["pixel_values_tgt"].to(accelerator.device, dtype=weight_dtype)
                    
                    # Check if this is CoT data with ground video
                    has_ground = "pixel_values_ground" in batch
                    if has_ground:
                        pixel_values_ground = batch["pixel_values_ground"].to(accelerator.device, dtype=weight_dtype)
                    
                    # Encode source, ground (if exists), and target separately
                    with torch.no_grad():
                        ...
                        # Encode source
                        latents_src = _batch_encode_vae(pixel_values_src)
                        
                        # Encode ground if exists (CoT data)
                        if has_ground:
                            latents_ground = _batch_encode_vae(pixel_values_ground)
                        
                        # Encode target
                        latents_tgt = _batch_encode_vae(pixel_values_tgt)
                        
                        # Concatenate along frame dimension for full attention
                        if has_ground:
                            # CoT: src + ground + tgt
                            # print('latents_src',latents_src.shape)
                            # print('latents_ground',latents_ground.shape)
                            # print('latents_tgt',latents_tgt.shape)
                            latents = torch.cat([latents_src, latents_ground, latents_tgt], dim=2)  # B, C, F_src+F_ground+F_tgt, H, W
                        else:
                            # Regular: src + tgt
                            latents = torch.cat([latents_src, latents_tgt], dim=2)  # B, C, F_src+F_tgt, H, W
                        
                        # Convert frame_split_indices from pixel space to latent space
                        # VAE temporal compression: (F-1)//4 + 1 for frames > 1, or 1 for single frame
                        if "frame_split_indices" in batch:
                            latent_frame_split_indices = []
                            pixel_indices = batch["frame_split_indices"]
                            for f_pixel in pixel_indices:
                                # Apply same temporal compression as VAE
                                if f_pixel == 1:
                                    f_latent = 1
                                else:
                                    f_latent = (f_pixel - 1) // vae.config.temporal_compression_ratio + 1
                                latent_frame_split_indices.append(f_latent)
                            # Store in batch for transformer
                            batch["frame_split_indices"] = latent_frame_split_indices
                        
                        # Convert ground_frame_indices from pixel space to latent space

                        if "ground_frame_indices" in batch and len(batch["ground_frame_indices"]) > 0:
                            latent_ground_frame_indices = []
                            
                            # 确保 split_indices 和 ground_indices 的数量匹配
                            num_samples = len(batch["frame_split_indices"])  # 已是 latent 起始位置
                            if num_samples != len(batch["ground_frame_indices"]):
                                raise ValueError("Batch size mismatch between frame_split_indices and ground_frame_indices")

                            for i in range(num_samples):
                                # 1) 已在上一步把 frame_split_indices 从像素帧数映射到 latent 帧数
                                latent_start = batch["frame_split_indices"][i]  # e.g., 33->9

                                # 2) 取当前样本的像素级 ground 区间，并计算像素长度
                                ground_start_pixel, ground_end_pixel = batch["ground_frame_indices"][i]
                                ground_pixel_length = ground_end_pixel - ground_start_pixel  # e.g., 38-33=5

                                # 3) 像素长度映射到 latent 长度（MagVAE: (F-1)//4+1）
                                if ground_pixel_length == 1:
                                    latent_ground_length = 1
                                else:
                                    latent_ground_length = (ground_pixel_length - 1) // vae.config.temporal_compression_ratio + 1  # e.g., 5->2

                                # 4) 计算 latent 结束位置
                                latent_end = latent_start + latent_ground_length  # e.g., 9+2=11

                                latent_ground_frame_indices.append((latent_start, latent_end))

                            # 写回 batch，供 RoPE 使用
                            batch["ground_frame_indices"] = latent_ground_frame_indices
                
                # Non-paired data branch
                else:
                ...
                # Split frame count for tracking (only for paired data)
                if is_paired_data:
                    num_frames_src = latents_src.size(2)
                    if has_ground:
                        num_frames_ground = latents_ground.size(2)
                    num_frames_tgt = latents_tgt.size(2)
                ...
                # For paired data, use frame_split_indices (already in latent space)
                # For non-paired data, use default calculation
                if is_paired_data and "frame_split_indices" in batch:
                    # Use the first sample's split index (all samples in batch should have same type)
                    edited_start_frame = batch["frame_split_indices"][0]
                else:
                    # Fallback for non-paired data
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    # For paired data, pass frame split indices for independent temporal RoPE
                    frame_split_indices = batch.get("frame_split_indices", None) if is_paired_data else None
                    ground_frame_indices = batch.get("ground_frame_indices", None) if is_paired_data else None
    ...
    # Run final DDP validation if requested
    run_final_ddp_validation(
        vae,
        text_encoder,
        tokenizer,
        clip_image_encoder if args.train_mode != "normal" else None,
        transformer3d,
        network,
        config,
        args,
