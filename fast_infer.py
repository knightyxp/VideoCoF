...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fast 4-step VideoCoF inference (single video or JSON list)"
    )
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        default="/scratch3/yan204/models/Wan2.1-T2V-14B",
    ...
    parser.add_argument("--lora_path", type=str, default=None, help="Path to custom LoRA checkpoint")
    parser.add_argument(
        "--enable_acceleration_lora",
        action="store_true",
        help="Enable loading the acceleration (FusionX) LoRA",
    )
    parser.add_argument(
        "--acceleration_lora_path",
        type=str,
        default=None,
        help="Optional path to acceleration LoRA; defaults to FusionX under model directory",
    )
...
def main():
    ...
    if rank == 0:
        print(f"Running parallel FAST CoT inference with {world_size} GPUs")
    ...
    if rank == 0:
        print(
            f"Total items: {len(items)}, already generated: {len(items) - len(pending_items)}, "
